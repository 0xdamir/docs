...
- `_autoParams` is the encoded `SubmissionAutoParamsFrom` struct, which is the derivative of the `SubmissionAutoParamsTo` with `nativeSender` taken from the
  Se`nt event: so you need to decode `_autoParams` against the `SubmissionAutoParamsTo` struct, add the fifth element, and encode back against the
...
![Gathering Data for Claims](/images/gathering-claims)
